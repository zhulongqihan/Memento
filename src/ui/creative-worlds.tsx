...
const presentFieldSource = `vec3 presentField(vec2 uv, vec2 pointer, float time) { float aspect = u_resolution.x / max(u_resolution.y, 1.0); vec2 p = vec2(uv.x * aspect, uv.y); vec2 center = vec2(aspect * 0.52, 0.48); vec2 q = p - center; float radius = length(q); float angle = atan(q.y, q.x); float rings = 0.5 + 0.5 * sin(radius * 42.0 - time * 0.48 + sin(angle * 4.0) * 0.8); rings *= smoothstep(1.15, 0.1, radius); float horizon = lineGlow(q.y + sin(q.x * 5.0 + time * 0.2) * 0.025, 0.0035); vec2 cursor = vec2(pointer.x * aspect, 1.0 - pointer.y) - p; float lens = exp(-length(cursor) * 8.0); float flare = pow(max(0.0, cos(atan(cursor.y, cursor.x) - time * 0.07)), 12.0) * lens; float grain = hash21(floor(uv * u_resolution * 0.18) + floor(time * 0.5)) * 0.025; vec3 ink = vec3(0.012, 0.018, 0.019); vec3 teal = vec3(0.04, 0.34, 0.28) * rings * 0.85; vec3 amber = vec3(0.74, 0.30, 0.10) * flare; vec3 pearl = vec3(0.72, 0.63, 0.46) * horizon * 0.48; return ink + teal + amber + pearl + grain; }`
